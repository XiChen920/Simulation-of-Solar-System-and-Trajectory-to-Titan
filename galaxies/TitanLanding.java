...
/**
 * A class which sets the scale of the titan landing universe itself
 *
 */
...
public class TitanLanding {
	...
	public TitanLanding(Handler handler) {
	...
	private void gridMaker(Graphics2D g) {
		...
		for(int i = -repetitions; i <= repetitions; i++) {
			...
			g.drawString((2*i*interval/10)+"m",(int) (5 + (2*i*interval) - handler.getEngineCamera().getxOffset()), (int) (-10 - handler.getEngineCamera().getyOffset())); 
			...
			//Vertical
			g.setColor(strongWhite);
			g.drawString((-2*i*interval/10)+"m",(int) (5 - handler.getEngineCamera().getxOffset()), (int) ((2*i*interval) - 10 - handler.getEngineCamera().getyOffset()));
